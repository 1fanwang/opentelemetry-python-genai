...
def _has_explicit_agent_override(
    metadata: dict[str, Any] | None,
) -> bool:
    return bool(metadata and metadata.get(_META_AGENT_SPAN))


def _has_explicit_workflow_override(
    metadata: dict[str, Any] | None,
) -> bool:
    return bool(metadata and metadata.get(_META_WORKFLOW_SPAN))
...
def classify_chain_run(
    ...
    if (
        ...
        or _has_explicit_agent_override(metadata)
    ):
        return OperationName.INVOKE_AGENT

    if _has_explicit_workflow_override(metadata):
    ...
    if announced_workflow or _looks_like_workflow(
        serialized, metadata, parent_run_id
    ):
        return OperationName.INVOKE_WORKFLOW
