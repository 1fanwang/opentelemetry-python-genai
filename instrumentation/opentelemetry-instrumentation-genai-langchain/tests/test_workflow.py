...
@pytest.mark.xfail(
    strict=True,
    reason="The LangChain async callback path loses nested span context.",
)
@pytest.mark.asyncio
async def test_async_nested_graph_preserves_parent_context(
    tracer_provider,
    meter_provider,
    logger_provider,
    span_exporter,
    ...
    with instrument(
        ...
        await _nested_graph().ainvoke(
            {"messages": [HumanMessage(content="hello")]}
        )

    workflow_spans_by_name = {
        span.attributes[GenAI.GEN_AI_WORKFLOW_NAME]: span
        for span in _workflow_spans(span_exporter)
    }
    inner_span = workflow_spans_by_name["named_subgraph"]
    outer_span = workflow_spans_by_name["LangGraph"]
    assert inner_span.parent.span_id == outer_span.context.span_id
...
def test_nested_graph_under_agent_metadata_is_workflow(
    ...
    workflow_span = next(
        span
        for span in spans
        if span.name == "invoke_workflow named_subgraph"
    )
    assert workflow_span.parent is not None


def test_workflow_run_name_overrides_graph_name(
    tracer_provider,
    meter_provider,
    logger_provider,
    span_exporter,
    ...
    with instrument(
        ...
        _graph(_respond).with_config(run_name="custom_workflow").invoke(
            {"messages": [HumanMessage(content="hello")]}
        )

    span = span_exporter.get_finished_spans()[0]
    assert span.name == "invoke_workflow custom_workflow"
    assert (
        span.attributes[GenAI.GEN_AI_WORKFLOW_NAME] == "custom_workflow"
    )
